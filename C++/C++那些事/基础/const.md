...
[const对象默认为文件局部变量](#const对象默认为文件局部变量)
[指针与const](#指针与const)
[函数中使用const](#函数中使用const)
[类中使用const](#类中使用const)
...
```
- 类型检查
const定义的变量只有类型为整数或枚举，且以常量表达式初始化时才能作为常量表达式。其它情况下他只是一个const限定的变量，不要和常量混淆。
- 防止修改，起保护作用，增加程序健壮性
- 节省空间，避免不必要的内存分配


## const对象默认为文件局部变量
<font style="color:red">注意：非const变量默认为extern。要使const变量能够在其他文件中访问，必须在文件中显式地指定它为extern。</font>
...
extern int ext;
int main(){
    std::cout<<(ext+10)<<std::endl;//输出11，可以访问file1中ext
}
```
>const常量在不同文件的访问
```C++
...
```
<font style="color:red">小结：可以发现未被const修饰的变量不需要extern显式声明！而const常量需要显式声明extern，并且需要做初始化！因为常量在定义后就不能被修改，所以定义时必须初始化。</font>
...
> 但是函数void func(A &a) 存在一个缺点：<br><br>“引用传递”有可能改变参数a，这是我们不期望的。解决这个问题很容易，加const修饰即可，因此函数最终成为 ``void func(const A &a)``。
...
使用const关键字声明的成员函数，称为==常成员函数==。只有常成员函数才有资格操作常量或常对象，没有使用const关键字进行说明的成员函数不能用来操作常对象。
